...
agnes <- G_scaled %>% 
  ...
  agnes( #выбираем Agglomerative clustering (иерархическая кластеризация снизу вверх)
    stand = F, #стандартизируем данныe
    metric = "euclidean", 
    method = "ward")

fviz_dend(agnes, 
          cex = 0.5, k = clusters_number,
          rect = TRUE,
          k_colors = "jco",
          show_labels = FALSE,
...
G_scaled %>% 
  kmeans(centers = clusters_number, # Кол-во центроидов-кластеров
         iter.max = 20, # Максимальное кол-во итераций
         nstart = 35) %>% # Кол-во центройдов в начале
  fviz_cluster(data = G_scaled,
