...
bac_functions <- read_xlsx (path, 1) %>% 
  ...
  mutate ("NA" = NULL) %>%
  pivot_wider(names_from = Neuromediator, values_from = Neuromediator) %>% 
  mutate ("NA" = NULL) %>% 
  pivot_wider (names_from = Vitamin, values_from = Vitamin) %>% 
  mutate ("NA" = NULL) %>%
  unchop(everything()) %>% 
  unique() %>% #удаление повторяющихся строк
  filter (! (Taxon %in% c("Blautia obeum_S", "Paenibacillus_G", "Oxalobacter_G"))) #bac_functions$Taxon[duplicated(bac_functions$Taxon)] для данных таксонов в исходном файле указана противоречивая информация
...
write_rds(data_long, 
          ...
          compress = "gz" ) 
#data_long не загружается на GitHub (слишком большой), поэтому был сжат
