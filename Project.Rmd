...
comparison_in_total %>% 
  mutate (p.unadjusted = ifelse(p.unadjusted < 0.001, "< 0.001", round (p.unadjusted,3)),
          p.value.holm = ifelse(p.value.holm < 0.001, "< 0.001", round (p.value.holm,3))
) %>% 
  flextable() %>% theme_box() %>% 
  align(align = "center", part = "all") %>% 
...
model_ser_G_1 <- glmer(Health_state ~ Серотонин +  (1 | research_ID) + (1 | patient_ID), data = G_long_serotonin, family = binomial)
summary(model_ser_G_1)

model_ser_G_2 <- glmer(Health_state ~ Серотонин +  (1 | Seq_date) + (1 | patient_ID), data = G_long_serotonin, family = binomial)
summary(model_ser_G_2)

anova(model_ser_G_1, model_ser_G_2)



G_long_serotonin_2 <- G_long_1 %>%
  filter(Percentage > 0.0000) %>%
  filter(`Серотонин` == "produce")


model_ser_G_3 <- glmer(Health_state ~ Percentage +  (1 | Seq_date) + (1 | patient_ID), data = G_long_serotonin_2, family = binomial)
summary(model_ser_G_3)

```

Ни одна из mixed models не показывает значимой ассоциации наличия серотонинпродуцирующих бактерий и состоянием здоровья человека (СРК/здоровый).
